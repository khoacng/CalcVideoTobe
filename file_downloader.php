...
if(isset($_POST['youtube']))
{
	...
	*/ 
	...
	$request = 'https://www.googleapis.com/youtube/v3/search?maxResults=5&order=viewCount&q=youtube.com&type=video&part=snippet&key='.$apiKey;
	...
	foreach($content['items'] as $item) {
	    array_push($list_id, $item['id']['videoId']);
    }
    foreach($list_id as $item) {
        $YouTubeId = $item;
        $request = 'https://www.googleapis.com/youtube/v3/videos?id='.$YouTubeId.'&part=snippet,contentDetails&key='.$apiKey;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_URL, $request);
